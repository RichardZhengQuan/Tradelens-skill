...
class StorageParserTest(unittest.TestCase):
    ...
    def test_history_save_redacts_secrets(self):
        with tempfile.TemporaryDirectory() as tmp:
            store = HistoryStore(Path(tmp))
            created_at = datetime(2026, 4, 27, 12, 0, 0)

            path = store.save("secret test", "POLYGON_API_KEY: abcdefghijklmnopqrstuvwxyz\nbroker password: hunter222", created_at)
            text = path.read_text(encoding="utf-8")

        self.assertNotIn("abcdefghijklmnopqrstuvwxyz", text)
        self.assertNotIn("hunter222", text)
    ...
    def test_mixed_stock_purchase_text_parses_as_unconfirmed_stock_order(self):
        order = parse_order_lines("@buy nvda now, 100 stocks")[0]

        self.assertEqual(order.action, "buy")
        self.assertEqual(order.symbol, "NVDA")
        self.assertEqual(order.quantity, 100)
        self.assertEqual(order.instrument, "stock")
        self.assertEqual(order.order_price, None)

    def test_chinese_stock_purchase_text_parses_as_unconfirmed_stock_order(self):
        order = parse_order_lines("现在买入 NVDA 100 股")[0]

        self.assertEqual(order.action, "买入")
        self.assertEqual(order.symbol, "NVDA")
        self.assertEqual(order.quantity, 100)
        self.assertEqual(order.instrument, "stock")
        self.assertEqual(order.order_price, None)
