...
class YahooProvider(MarketDataProvider):
    ...
    def get_quote(self, symbol: str):
        ...
        try:
            quote = self._fetch_quote_cached(symbol, DEFAULT_TTLS["quote"])
    ...
    def get_volatility(self):
        ...
        vix = uvix = None
        errors = []
        for target, quote, error in self._fetch_quotes_parallel(("^VIX", "UVIX"), ttl_seconds=DEFAULT_TTLS["vix"]):
            if error:
    ...
    def get_index_context(self):
        ...
        errors = []
        attrs = {"SPY": "spy", "QQQ": "qqq", "SOXX": "soxx"}
        for symbol, quote, error in self._fetch_quotes_parallel(tuple(attrs), ttl_seconds=DEFAULT_TTLS["index_context"]):
            if error:
                errors.append(f"{symbol}: {error}")
                context.missing_fields.append(f"{symbol} latest context")
                continue
            setattr(context, attrs[symbol], quote)
    ...
    def _fetch_quote_cached(self, symbol: str, ttl_seconds: int | float) -> QuoteSnapshot:
        cache = self.cache
        key = f"quote:{symbol.upper()}"
        if cache is None:
            return self._fetch_quote(symbol)
        return cache.get_or_set(key, ttl_seconds, lambda: self._fetch_quote(symbol))

    def _fetch_quotes_parallel(self, symbols: tuple[str, ...], ttl_seconds: int | float):
        results = []
        with ThreadPoolExecutor(max_workers=len(symbols)) as executor:
            future_by_symbol = {
                executor.submit(self._fetch_quote_cached, symbol, ttl_seconds): symbol
                for symbol in symbols
            }
            for future in as_completed(future_by_symbol):
                symbol = future_by_symbol[future]
                try:
                    results.append((symbol, future.result(), None))
                except Exception as exc:  # pragma: no cover - network behavior is host-dependent
                    results.append((symbol, None, exc))
        return results
