...
ACTION_RE = re.compile(
    r"\b(sell\s+to\s+open|buy\s+to\s+close|buy\s+to\s+open|sell\s+to\s+close|buy|sell|bto|sto|btc|stc|short|cover)\b|买入|卖出|买|卖",
    re.IGNORECASE,
)
...
STOCK_ORDER_ACTION_FIRST_RE = re.compile(
    r"""
    ^\s*@?\s*
    .*?
    (?P<action>(?:buy|sell)\b|买入|卖出|买|卖)
    .*?
    (?P<symbol>[A-Za-z]{1,6})
    .*?
    (?P<quantity>\d+(?:\.\d+)?)
    \s*
    (?P<unit>shares?|stocks?|股)
    .*
    $
    """,
    re.IGNORECASE | re.VERBOSE,
)
STOCK_ORDER_SYMBOL_FIRST_RE = re.compile(
    r"""
    ^\s*@?\s*
    .*?
    (?P<symbol>[A-Za-z]{1,6})
...
def _normalize_action(action: str) -> str:
    ...
    return {"买": "买入", "卖": "卖出"}.get(normalized, normalized)


def _parse_stock_order(line: str) -> dict:
    match = STOCK_ORDER_ACTION_FIRST_RE.match(line) or STOCK_ORDER_SYMBOL_FIRST_RE.match(line)
    if not match:
        return {}
    groups = match.groupdict()
    return {
...
def extract_symbol(text: str) -> str:
    ...
    if not parsed:
        parsed = _parse_stock_order(text.strip())
    if parsed:
...
def parse_order_lines(text: str, prefix: str = "ORD") -> List[OrderLine]:
    ...
    for index, line in enumerate([l.strip() for l in text.splitlines() if l.strip()], start=1):
        parsed = _parse_broker_order(line) or _parse_stock_order(line)
        ...
        orders.append(
            OrderLine(
                ...
                action=parsed.get(
                    "action",
                    _normalize_action(action_match.group(0)) if action_match else "unknown",
                ),
