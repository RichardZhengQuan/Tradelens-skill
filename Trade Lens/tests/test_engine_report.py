...
from tradelens.analysis.report_writer import STANDARD_DISCLAIMER, write_analysis_report
...
from tradelens.models import MarketSnapshot, Scenario, ScoreBreakdown, TradeIntent, TradeTerm
...
class EngineReportTest(unittest.TestCase):
    ...
    def test_report_does_not_include_action_recommendation_language(self):
        result = AnalysisEngine(self._fresh_provider()).analyze_trade(
            symbol="NVDA",
            question="you should buy, you should sell, you should wait; best action recommend",
            scenarios=REQUESTED_SCENARIOS,
            max_data_age_minutes=15,
        )
        report = write_analysis_report(result)
    ...
    def test_structured_trade_judgment_is_lowercase(self):
        ...
        self.assertRegex(report, r"(?m)^- trade_judgment: [a-z ]+$")
